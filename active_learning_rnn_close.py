...
def create_keras_model():
    ...
    model.add(SimpleRNN(32,return_sequences=True,input_shape=(cf.ROW_LENGTH,1)))
...
def get_close_data(x_list):
    ...
    xxxx = (xxx-mean)/mean
    xxxxx = np.reshape(xxxx,[-1,cf.ROW_LENGTH,1])
    return xxxxx
...
x_initial_close = get_close_data(X_initial)
learner = ActiveLearner(
    estimator=classifier,
    query_strategy=regression,
    X_training=x_initial_close[:,::-1,:], y_training=y_initial,
    verbose=0
)
classifier.save(cf.GENERAL_MODEL_NAME)
...
x_not_order_close = get_close_data(x_not_order)
learner.teach(
    X=x_not_order_close[:,::-1,:], y=y_not_order
)
x_order_close = get_close_data(x_order)
for i in range(cf.ORDER_TO_NOT_ORDER):
    learner.teach(
        X=x_order_close[:,::-1,:], y=y_order
    )
classifier.save(cf.GENERAL_MODEL_NAME)
print("inital OK")
exit(0)
...
for idx in range(n_queries):
    x_pool_close = get_close_data(x_pool)
    query_idx, query_instance = learner.query(x_pool_close[:,::-1,:])
    xx,xxxx = plt_manual(query_idx)

    print(query_idx)
    if len(xx) != 0:
        learner.teach(
            X=x_pool_close[xx][:,::-1,:], y=keras.utils.to_categorical(y_pool[xx],4)
        )
    if len(xxxx) != 0:
        for i in range(cf.ORDER_TO_NOT_ORDER):
            learner.teach(
                X=x_pool_close[xxxx][:,::-1,:], y=keras.utils.to_categorical(y_pool[xxxx],4)
            )
